...
Item {
  ...
  readonly property bool canSend: !busy && !uncertain && validServer && (!inference || inferenceModel!==null) && message.trim().length > 0 && (projectId === "" || (selectedProject !== null && !selectedProject.unavailable))
  ...
  function open(payload) {
    ...
    opened=true
    if (!busy && !recoveryPending) { status=""; uncertain=false;taskId="";message="";inference="" }
